...
//        NSLog(@"\n *******use count is %d ,less equal the APPIRATER_USES_UNTIL_PROMPT %d,return NO",useCount,APPIRATER_USES_UNTIL_PROMPT);
//        return NO;
//    }
	...
	if (sigEventCount <= APPIRATER_SIG_EVENTS_UNTIL_PROMPT){
         NSLog(@"\n *******sigEventCount is %d ,less equal the APPIRATER_SIG_EVENTS_UNTIL_PROMPT %d,return NO",sigEventCount,APPIRATER_SIG_EVENTS_UNTIL_PROMPT);
        return NO;
    }
	
	// has the user previously declined to rate this version of the app?
	if ([userDefaults boolForKey:kAppiraterDeclinedToRate]){
        NSLog(@"\n *******BOOL key kAppiraterDeclinedToRate is %@ ,user declined rating ,return NO",kAppiraterDeclinedToRate);
        return NO;
    }
	
	// has the user already rated the app?
	if ([userDefaults boolForKey:kAppiraterRatedCurrentVersion]){
         NSLog(@"\n *******BOOL key kAppiraterRatedCurrentVersion is %@ ,user have rated cur version ,return NO",kAppiraterRatedCurrentVersion);
		return NO;
    }
	...
	if (timeSinceReminderRequest < timeUntilReminder){
        NSLog(@"\n ******user have selected remind later,less equal the APPIRATER_TIME_BEFORE_REMINDING :%d ,return NO",APPIRATER_TIME_BEFORE_REMINDING);
        return NO;
    }
	
    
    NSLog(@"AppIrater config is \n sigEventCount : %d\n userHaveDeclinedToRate : %@\n userHaveRatedCurrentVersion : %@ \n ^^^^^^^^^^^^^^^congrats!review alert popup^^^^^^^^^^^^^^^",sigEventCount,([userDefaults boolForKey:kAppiraterDeclinedToRate])==NO?@"NO":@"YES",([userDefaults boolForKey:kAppiraterRatedCurrentVersion])==NO?@"NO":@"YES");
	return YES;
}
...
+ (void)appLaunched:(BOOL)canPromptForRating {
    ...
    NSLog(@"AppIrater config is :\n APPIRATER_DAYS_UNTIL_PROMPT : %d\n APPIRATER_USES_UNTIL_PROMPT : %d\n APPIRATER_SIG_EVENTS_UNTIL_PROMPT : %d\n APPIRATER_TIME_BEFORE_REMINDING : %d days(day)\n",APPIRATER_DAYS_UNTIL_PROMPT,APPIRATER_USES_UNTIL_PROMPT,APPIRATER_SIG_EVENTS_UNTIL_PROMPT,APPIRATER_TIME_BEFORE_REMINDING);
}
